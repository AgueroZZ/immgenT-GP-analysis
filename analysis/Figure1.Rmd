...
```{r fig1c-code, code=readLines("../script/Figure1.R")[109:193], eval=FALSE}
```
...
connected to its **top 10 up-regulated signature genes** (largest positive
normalized loadings, ≥ 0.1). **Blue balls are GPs** (numbered) and **tan balls
are genes**; an edge joins a GP to each of its top signature genes, so a gene
that appears among several GPs' top genes links those GPs. Five **marker genes**
(legend: *Foxp3, Cd8a, Cd8b1,
Izumo1r* [FR4], *Il2ra*) are highlighted, each in its own color shared by the
gene's ball and its edges, indicating which GPs carry that marker.
:::

## (D) GP1 signature volcano {#fig1d}

```{r fig1d-code, code=readLines("../script/Figure1.R")[294:300], eval=FALSE}
```
...
```{r fig1e-code, code=readLines("../script/Figure1.R")[204:219], eval=FALSE}
```
...
```{r fig1f-code, code=readLines("../script/Figure1.R")[274:283], eval=FALSE}
```
...
```{r fig1g-code, code=readLines("../script/Figure1.R")[239:258], eval=FALSE}
```
...
```{r fig1h-code, code=readLines("../script/Figure1.R")[221:237], eval=FALSE}
```
...
```{r fig1i-code, code=readLines("../script/Figure1.R")[260:272], eval=FALSE}
```
