...
::: {.figcaption}
**Fig. S1D.** Heatmap of the per-IGT mean loading for the ten GPs with the highest between-IGT variance in (C) -- the same ten GPs labelled there, ranked on the same all-IGT variance. Columns are restricted to the IGTs with >= 500 standard-spleen cells, so that no column is a mean over a handful of cells. Rows (GPs) are hierarchically clustered; color runs from white (low) to red (high mean loading).
:::
