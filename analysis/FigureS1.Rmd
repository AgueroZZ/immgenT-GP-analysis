...
```{r figs1e-code, code=readLines("../script/FigureS1.R")[165:195], eval=FALSE}
```
...
::: {.figcaption}
**Fig. S1E.** For each GP, the expected number of active genes (x-axis) versus the expected proportion of active cells (y-axis), taken directly from the EBMF-learned point-mass prior of each factor (the mixture weight on the non-zero component, `1 - pi0`, with the gene x-axis rescaled to a count by the total number of genes) rather than from a hard threshold on the fitted matrices. Both axes on log scale; each dot is one of the 200 GPs. GPs that are active in more genes tend also to be active in more cells.
:::
