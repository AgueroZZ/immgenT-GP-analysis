...
Panels (s3c)-(s3h) are produced by
...
## (s3a, s3b) Definition of resting and activated CD4/CD8 cells {#figs3ab}

These two panels are **missing from this site**: no code in
`script/FigureS3.R` produces them, and there is no `s3a`/`s3b` asset under
`analysis/assets/FigureS3/`, `figures/final-selected/Figure S3/`, or
`figures/Previous/bits/`. The published captions describe them as follows.
...
highlighted in each lineage-specific MDE.

**Fig. S3b.** Gated cells are shown in the adjacent CD62L versus CD44
protein-expression plot (CITE-seq; log-normalized ADT counts).
:::

The `annotation_level2_group` "activated"/"resting" labels that every other panel
on this page (and all of [Figure 4](Figure4.html)) filters on are the labels these
two panels define, so the gate itself is already in use here -- only the two
...
::: {.figcaption}
**Fig. S3c.** Bar plot showing the percentage of **activated CD4+ and CD8+ T cells** in each organ (`organ_simplified`) with GP26 activity above **0.1**, sorted from highest to lowest. Activated cells are those with `annotation_level2_group == "activated"` among the CD4 and CD8 lineages.
:::
...
```{r figs3h-code, code=code_for("../script/FigureS3.R", "s3h"), eval=FALSE}
```
