...
3. `02_compute_auc.R` — per-GP AUC for predicting lineage/organ/condition
   (feeds Figure2, Figure4, Table S1).
...
| `level_1_AUC_list_figure_no_thymocytes_healthy.rds`, `level_2_AUC_list_figure_no_thymocytes_healthy.rds`, `organ_simplified_AUC_list_figure_no_thymocytes_healthy.rds` | `02_compute_auc.R` | Per-GP AUC/threshold for predicting level-1/level-2/organ, restricted to non-thymocyte healthy cells. Used by Figure2 (2A), Figure4, and TableS1 -- all three were previously reading two different, disagreeing cached files (see below); now consistent. |
| `level_1_AUC_list_figure.rds`, `level_2_AUC_list_figure.rds`, `organ_simplified_AUC_list_figure.rds` (no suffix) | *(superseded, not read by anything)* | **Not** the same computation as the `_no_thymocytes_healthy` files despite the near-identical name (max abs AUC diff up to 0.46 against the correct version) -- confirmed to be missing the healthy-only restriction (computed on all non-thymocyte cells, healthy and diseased together), not a stale-annotation issue as first suspected: rerunning with just that restriction dropped reproduces these files exactly (`organ_simplified`: byte-identical; `level_1`/`level_2`: diff ~1e-12, floating-point noise). The no-suffix `organ_simplified` file's 4 extra categories (SLO, prostate, pancreas, synovial fluid) are disease-model-specific sample sites, consistent with this. Figure2.R and TableS1.R used to read these; both were switched to the `_no_thymocytes_healthy` files. Left in `data/` untouched (superseded, no longer referenced). |
| `condition_detailed_AUC_list_figure.rds` | *(dropped, not read by anything)* | TableS1 used to have a Condition column from this file, grouped by `condition_detailed` and restricted to non-thymocyte cells only (not healthy-only, since condition_detailed is the condition variable itself -- so the "missing healthy filter" explanation above doesn't apply here). Whatever's actually wrong with this cached file (dated Dec 2 2025, predating even the current Seurat object) was never independently root-caused, since the Condition column was dropped from TableS1.R entirely rather than investigated further. |
