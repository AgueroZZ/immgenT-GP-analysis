...
| `level_1_AUC_list_figure.rds`, `level_2_AUC_list_figure.rds`, `organ_simplified_AUC_list_figure.rds` (no suffix) | *(superseded, not read by anything)* | Turned out to be a stale artifact of an older script (`analysis/old/Figures_Manuscript_v1.rmd`) that read the now-known-stale cached `seurat_meta.rds` and didn't restrict to healthy cells -- **not** the same computation as the `_no_thymocytes_healthy` files despite the near-identical name (max abs AUC diff up to 0.095 against the correct version). Figure2.R and TableS1.R used to read these; both were switched to the `_no_thymocytes_healthy` files. Left in `data/` untouched (superseded, no longer referenced). |
| `condition_detailed_AUC_list_figure.rds` | *(dropped, not read by anything)* | TableS1 used to have a Condition column from this file, grouped by `condition_detailed` and restricted to non-thymocyte cells only (not healthy-only, since condition_detailed is the condition variable itself). The cached file turned out to have the same stale-`seurat_meta.rds` problem as the level_1/2/organ files above. Rather than recompute (this one is the expensive one -- 127 categories, ~30+ min), the Condition column was dropped from TableS1.R entirely. |
