...
build_cosine_score_matrix_qs_hungarian <- function(
  ...
  K_all <- ncol(F_all)
  score_mat <- matrix(0, nrow = K_all, ncol = length(files),
                      dimnames = list(colnames(F_all), paste0("IGT", igt_ids)))
  match_mat <- matrix(NA_integer_, nrow = K_all, ncol = length(files),
                      dimnames = list(colnames(F_all), paste0("IGT", igt_ids)))
  unmatched_mat <- matrix(FALSE, nrow = K_all, ncol = length(files),
                          dimnames = list(colnames(F_all), paste0("IGT", igt_ids)))
...
write.csv(score_mat, file.path(data_path, "igt_specific_cosine_scores.csv"),
          row.names = TRUE)
...
write.csv(validated, file.path(data_path, "igt_specific_validated_matrix.csv"),
          row.names = TRUE)
...
# save as pdf
ggsave(file.path("igt_specific_validated_by_X_IGTs.pdf"),
       width = 6, height = 4)
...
ggplot(plot_df_non_cum, aes(x = X, y = n_GP, color = factor(threshold), group = factor(threshold))) +
  geom_line() +
  geom_point(size = 1) +
  # log scale for y-axis if needed
  scale_y_log10() +
...
ggsave(file.path("igt_specific_validated_by_exactly_X_IGTs.pdf"),
       width = 6, height = 4)
...
# ---- plot ----
ggplot(plot_df, aes(x = n_IGTs_included, y = validated_GPs, color = threshold)) +
...
# save as pdf
ggsave(file.path("igt_specific_cumulative_validated.pdf"),
       width = 6, height = 4)
