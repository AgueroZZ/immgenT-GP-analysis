...
(the file is plain CSV, all 200 GPs pre-listed so no GP name has to be typed),
then re-run
...
The file is UTF-8, and five comments (GP3, GP6, GP7, GP8, GP29) contain Greek
characters -- the gamma/delta of "gd T", and `TCRVg3`. A .csv has nowhere to
record its encoding, so **do not edit this file by double-clicking it into
Excel**: Excel reads BOM-less UTF-8 as legacy 8-bit text and saving back turns
those characters into mojibake. Use a text editor, or Excel's *Data > From
Text/CSV* import with the encoding set to UTF-8, and check those five rows
afterwards. (This is why the published tables themselves are .xlsx -- see
`code/R/table_xlsx.R`.)
