"""Figure 7b matching step: pair each EBMF gene program with an RQVI program.
...
aggregated onto the 107 non-thymocyte clusters displayed in Figure 7b.
...
Run: python script/Figure7b_rematch.py
"""
...
# cell -> annotation table written by Figure7b.R (run step 1 first)
CELL_META = OUT_DIR / "7b_cell_metadata.csv.gz"
...
def main() -> None:
    ...
    # ---- display basis: non-thymocyte cells, order from 7b_cluster_order.csv ----
    order = pd.read_csv(OUT_DIR / "7b_cluster_order.csv").sort_values("display_column")
    ...
    if not np.array_equal(d_counts.astype(int), order["n_cells"].to_numpy()):
        raise ValueError("display cell counts differ from 7b_cluster_order")
    ...
    out.to_csv(OUT_DIR / "7b_rqvi_rematched_raw_means_level2.csv")
    ...
    }).to_csv(OUT_DIR / "7b_rematch_mapping.csv", index=False)
    ...
    print(f"wrote {OUT_DIR/'7b_rqvi_rematched_raw_means_level2.csv'} and 7b_rematch_mapping.csv")
