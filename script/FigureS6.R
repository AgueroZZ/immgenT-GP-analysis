# Figure S6. Protein programs across GPs.
#
# One panel (see analysis/FigureS6.Rmd for the caption text):
#   s6   Heatmap of the re-estimated protein matrix U (sparse: |score| < 0.5
...
#
# Split out of what is now Figure S7 on 2026-07-30. It was that figure's panel
# s6a (and before 2026-07-28, the published Figure 6b); it became a standalone
# figure in the Extended Data Figure 5 slot that main Figure 7b vacated, and
# moved to Extended Data Figure 6 on 2026-08-27 when the per-lineage structure
# plots took slot 5. For anyone diffing against figures/Previous/bits/:
#
#   now   was                    what
#   s6    Figure 6/6b            protein-program heatmap
#
...
# ============================================================
# s6: sparse protein-program heatmap, contamination GPs removed
# ============================================================
...
wide_matrix_s6 <- as.matrix(Protein_F_pm_simplified_no_contamination)
wide_visible_mask_s6 <- abs(wide_matrix_s6) >= sparse_cutoff
wide_gp_visible_count_s6 <- colSums(wide_visible_mask_s6)
wide_protein_visible_count_s6 <- rowSums(wide_visible_mask_s6)
wide_gp_number_s6 <- as.integer(sub("^GP", "", colnames(wide_matrix_s6)))

wide_gp_order_s6 <- order(-wide_gp_visible_count_s6, wide_gp_number_s6)
wide_mask_ordered_cols_s6 <- wide_visible_mask_s6[
  ,
  wide_gp_order_s6,
  drop = FALSE
]
wide_rightmost_visible_gp_s6 <- apply(
  wide_mask_ordered_cols_s6,
  1,
  function(values) max(which(values))
)
wide_rarity_weights_s6 <- seq_len(ncol(wide_mask_ordered_cols_s6))^2
wide_protein_rarity_score_s6 <- as.numeric(
  wide_mask_ordered_cols_s6 %*% wide_rarity_weights_s6
)
wide_protein_order_s6 <- order(
  -wide_rightmost_visible_gp_s6,
  -wide_protein_visible_count_s6,
  -wide_protein_rarity_score_s6,
  rownames(wide_matrix_s6)
)
wide_ordered_matrix_s6 <- wide_matrix_s6[
  wide_protein_order_s6,
  wide_gp_order_s6,
  drop = FALSE
]

pdf(paste0(figure_path, "s6.pdf"), width = 48, height = 14)
pheatmap::pheatmap(
  wide_ordered_matrix_s6,
  main = sprintf(
    paste0(
...
# ============================================================
expected_panels <- paste0(figure_path, "s6", ".pdf")
