# Figure S7. Protein programs across GPs.
#
# One panel (see analysis/FigureS7.Rmd for the caption text):
#   s7   Heatmap of the re-estimated protein matrix U (sparse: |score| < 0.5
...
#
# Split out of what is now Figure S8 on 2026-07-30. It was that figure's panel
# s6a (and before 2026-07-28, the published Figure 6b); it became a standalone
# figure in the Extended Data Figure 5 slot that main Figure 7b vacated, moved
# to Extended Data Figure 6 on 2026-08-27 when the per-lineage structure plots
# took slot 5, and to Extended Data Figure 7 on 2026-09-09 when a new
# cluster-level figure was inserted at 3. For anyone diffing against
# figures/Previous/bits/:
#
#   now   was                    what
#   s7    Figure 6/6b            protein-program heatmap
#
...
# gating inputs Figure 6 / Figure S7 use); it is sourced as-is rather than
...
# ============================================================
# s7: sparse protein-program heatmap, contamination GPs removed
# ============================================================
...
wide_matrix_s7 <- as.matrix(Protein_F_pm_simplified_no_contamination)
wide_visible_mask_s7 <- abs(wide_matrix_s7) >= sparse_cutoff
wide_gp_visible_count_s7 <- colSums(wide_visible_mask_s7)
wide_protein_visible_count_s7 <- rowSums(wide_visible_mask_s7)
wide_gp_number_s7 <- as.integer(sub("^GP", "", colnames(wide_matrix_s7)))

wide_gp_order_s7 <- order(-wide_gp_visible_count_s7, wide_gp_number_s7)
wide_mask_ordered_cols_s7 <- wide_visible_mask_s7[
  ,
  wide_gp_order_s7,
  drop = FALSE
]
wide_rightmost_visible_gp_s7 <- apply(
  wide_mask_ordered_cols_s7,
  1,
  function(values) max(which(values))
)
wide_rarity_weights_s7 <- seq_len(ncol(wide_mask_ordered_cols_s7))^2
wide_protein_rarity_score_s7 <- as.numeric(
  wide_mask_ordered_cols_s7 %*% wide_rarity_weights_s7
)
wide_protein_order_s7 <- order(
  -wide_rightmost_visible_gp_s7,
  -wide_protein_visible_count_s7,
  -wide_protein_rarity_score_s7,
  rownames(wide_matrix_s7)
)
wide_ordered_matrix_s7 <- wide_matrix_s7[
  wide_protein_order_s7,
  wide_gp_order_s7,
  drop = FALSE
]

pdf(paste0(figure_path, "s7.pdf"), width = 48, height = 14)
pheatmap::pheatmap(
  wide_ordered_matrix_s7,
  main = sprintf(
    paste0(
...
# ============================================================
expected_panels <- paste0(figure_path, "s7", ".pdf")
