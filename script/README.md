...
- writes its panels into `figures/final-selected/Figure N/` (or `Figure SN/`)
  under the *current* figure numbering, which from Figure 2 on is NOT the
...
Run any script from the repo root, e.g. `Rscript script/Figure5.R`.
...
| S2, panels a-c | `FigureS2.R` | `Figure_Lineage.R` |
| S2, panel d | `FigureS2d.R` | new (no published counterpart) |
| S3 | `FigureS3.R` | `Figure_Activation.R` |
| S4 | `FigureS4.R` | `Figure_CITEseq.R` (its panel a) |
| S5 | `FigureS5.R` | `Figure_CITEseq.R` + `gated_protein_loading_plot.R` |

The rest of Figure 8, and all of the **published** Figure S7 -- the RQVI figure,
published as Extended Data Figure 7, renumbered to 8 here on 2026-08-27 and back
to **7** on 2026-09-10 -- are **out of scope** (see below). Extended Data is
currently **1-7**: 1-6 are documented here, 7 in the companion RQVI repository.
...
- **The CITE-seq figure** (then Figure 6, now **Figure 7**) was reordered to
  a-j: the KLRG1 pair and the CD69 gene heatmap
  ...
  figure (then S6; now split across S5 and S6).
- **The CITE-seq Extended Data figure** stopped being a two-page gallery of 23
  ...
  gallery GPs were promoted into that figure; the remaining 17 are no longer
  published. Its first panel then became a figure of its own on 2026-07-30, and
  both were renumbered twice since (below): the heatmap is now **S5/s5** and
  the six remaining panels **S6/s6a-s6f**.
- **Figure S3**'s s3a and s3b were merged into one s3a (they were always one
  ...
  the figure is a-f.
- **The activation figure** (then Figure 4, now **Figure 5**; 2026-07-29)
  dropped its bipartite TF-GP network, the published 3e, so the two heatmaps
  after it moved up a letter: published 3f, 3g are now its c, d and the figure
  is a-d. That panel was the only caller anywhere of
  ...
  was removed.
- **Extended Data Figure 5** became main **Figure 8b** (then 7b), assembled from its two
  half-panels into one PDF, replacing a different collaborator panel.

## The 2026-07-30 CITE-seq split

The CD69/gating figure's first panel -- the protein-program heatmap, published as
Figure 6b -- became a standalone figure, taking the Extended Data Figure 5 slot
that main Figure 8b (then 7b) had vacated. The remaining panels each dropped one letter (the two
CD69 mean-activity heatmaps became a, b; the four gating panels c-f). Only letters
and file paths moved; no panel was re-rendered, so every PDF and site PNG is the
same file under a new name.

## The 2026-08-27 Extended Data renumbering

The per-lineage cluster structure plots were added as **Extended Data Figure 5**,
and the figures at 5-7 each moved back one:

| after this pass | before it | what |
|---|---|---|
...
Every one of those four moved again on 2026-09-10; see below for where they are
now.

The two CITE-seq figures were renamed, not re-rendered: every PDF and site PNG is
the same file under a new name (verified pixel-identical). Their scripts' internal
panel letters, the pages' chunk labels and anchors, the three CITE-seq
verification scripts and `verify_panels.sh`'s map all moved with them. Extended
Data Figure 8 keeps its number only in the companion repository's own
...
was retired. Its **cluster** heatmap survives as a figure of its own at
**Extended Data 3**, pushing the activation figure from 3 to **4**; its
all-200-GP **tissue** heatmap is gone from Extended Data altogether -- a 32-GP
version of it is now an internal draft in `experiments/fig_n5/`, a candidate
replacement for what is now main Figure 6. Extended Data 5-8 were unaffected and
kept the numbers they had before this pass.
...
| -- | S4's panel `s4a` | retired from Extended Data; see `experiments/fig_n5/` |

Nothing was re-rendered. The five activation panels and the cluster heatmap are
byte-identical under their new names, and the new `FigureS3.R` -- the retired
`FigureS4.R` with its tissue half removed -- reproduces `s4b` at RMSE 0. The
heatmap machinery that script defined inline (group means, centering,
dominant-group ordering, the ComplexHeatmap call) moved to
`code/R/centered_mean_heatmap.R`, which `experiments/fig_n5/` uses too.

## The 2026-09-10 Extended Data reorganisation

The cluster heatmap that had been Extended Data Figure 3 for one day became
**panel d of Extended Data Figure 2**, and everything after it moved up one, so
Extended Data is **1-7** again.

| now | was | what |
|---|---|---|
| **S2/S2D** `FigureS2d.R` -> `Figure S2/S2D.pdf` | S3 `FigureS3.R`, `s3.pdf` | all 200 GPs across the level-2 clusters |
| **S3** `FigureS3.R` -> `Figure S3/s3b-s3f.pdf` | S4 `FigureS4.R`, `s4b-s4f.pdf` | activation-associated GPs |
| **S4** `FigureS4.R` -> `Figure S4/s4.pdf` | S5 `FigureS5.R`, `s5.pdf` | seven per-lineage structure plots |
| **S5** `FigureS5.R` -> `Figure S5/s5.pdf` | S6 `FigureS6.R`, `s6.pdf` | protein-program heatmap |
| **S6** `FigureS6.R` -> `Figure S6/s6a-s6f.pdf` | S7 `FigureS7.R`, `s7a-s7f.pdf` | CD69 mean activity (a, b) + gating (c-f) |
| **7** *(companion RQVI repository)* | 8 | RQVI scalability and representative matches |

## The 2026-09-10 removal of the structure-plot figure

Ziang removed the seven per-lineage structure plots from Extended Data the same
day, and the two figures after it moved up one, so Extended Data is **1-6**:

| now | was | what |
|---|---|---|
| -- | S4, the seven per-lineage structure plots | **removed**; only its record survives |
| **S4** `FigureS4.R` -> `Figure S4/s4.pdf` | S5 `FigureS5.R`, `s5.pdf` | protein-program heatmap |
| **S5** `FigureS5.R` -> `Figure S5/s5a-s5f.pdf` | S6 `FigureS6.R`, `s6a-s6f.pdf` | CD69 mean activity (a, b) + gating (c-f) |
| **6** *(companion RQVI repository)* | 7 | RQVI scalability and representative matches |

The figure's script could not simply go: **Figure 4's panels b, c and d are its
GPs and its cells in another form, and `Figure4.R` stops if they disagree with
what it recorded.** So `script/FigureS4.R` became
**`script/structure_plot_record.R`**, which still builds the seven rows and
still writes the record -- now to `output/structure_plot_record/` as
`panel_gps.csv` and `panel_clusters.csv` -- but no longer assembles or saves a
PDF. The record it writes is byte-identical to the one the figure wrote.
`verify_structure_plot_gps.R` follows it, minus the two checks that died with
the figure (the assembled PDF on disk, and the caption's numbers).

The renamed figures were renamed, not re-rendered: their PDFs and site PNGs are
the same files under new names, and their scripts' panel letters, the pages'
chunk labels and anchors, the verification scripts and `verify_panels.sh`'s map
all moved with them.
...
panels cannot drift apart on what "activated" is coloured; that module also
holds `level2_group_block_order()`, the column rule above.
...
2026-09-10, shifting the three after it again: the published 2, 3, 4 and 6 are
our **3, 5, 6 and 7**. The CITE-seq figure, Figure S3 and the CD69/gating figure
(now S6) were re-lettered on 2026-07-28, the activation figure on 2026-07-29,
and the Extended Data figures renumbered on 2026-08-27, 2026-09-09 and
2026-09-10. A same-letter filename comparison is therefore wrong almost
everywhere -- and for S4/S5/S6 a same-*number* comparison is wrong too:
...
| 7e-7h | Figure 6/**6c-6f** (gating: GP171, GP12, GP80, GP23) |
| 7i, 7j | *(new panels -- gating for GP77 and GP8)* |
| S4/s4 | Figure 6/**6b** (protein-program heatmap; renumbered several times, S4 since 2026-09-10) |
| S5/s5a, s5b | Figure 6/**6j, 6k** (CD69 GPs by tissue / lineage; renumbered several times, S5 since 2026-09-10) |
| S5/s5c-s5f | *(new standalone panels; their GPs were only ever drawn inside the retired two-page gallery, `Previous/bits/Figure S6/s6-1` and `s6-2`)* |
| S3/s3b-s3f | Figure S3/**s3c-s3g** (published s3h, the TF-GP network, was dropped; the letters are off by one even though the number matches) |
| 8A, 8C-8G | Figure 7/**7A, 7C-7G** (straight copies, out of scope) |
...
`s6-2`), not our S5/s5a-s5f gating figure, and `Previous/bits/Figure S7/` is the
RQVI figure, which is our Extended Data Figure **6** and is out of scope.

`Previous/bits/Figure S3/` is the published activation figure and pairs with our
**S3** -- same number, but its letters are off by one (above). Our S2/S2D, the
cluster heatmap, has no published counterpart.
...
passed a panel that plotted the wrong variable (now 6d, 0.96% off in size) and a
gallery whose panels were permuted (now 7e-7g). Nor is md5: every regenerated PDF
embeds a fresh `/CreationDate`, so only the unmodified copies (1B, 7a, all of
Figure 8) ever match byte-for-byte. Compare **pixels**, against the panel the
mapping above says is the counterpart:

```
magick -density 110 "figures/final-selected/Figure 6/6d.pdf[0]" \
...
**A re-run can silently not write.** The CD69/gating script (now `FigureS5.R`)
...
here writes multi-megabyte `cairo_pdf()` output any more. `FigureS5.R` keeps a
...
that stamp (`FigureS4.R`, split out of it, and `structure_plot_record.R` carry the same guard
...
`threshold_df` that `Figure7.R:216` and `FigureS5.R:123` actually gate on, and
...
hand-picked, so the two captions describing it (Fig. 7d and Fig. S7a, b) and the
...
across two figures (7e-7j and s6c-s6f) and so across two scripts. It reads each
...
`verify_structure_plot_gps.R` covers the per-lineage structure-plot record,
whose *content* is a selection: each of its seven rows holds the GPs that reach
AUC > 0.9 for at least one cluster of its lineage, and those same AUCs are
published as Extended Data Table 6. Figure 4's b, c and d are built from that
record, so a drift here is a drift in a published figure. It reads the row map,
threshold and display filters from `code/R/structure_plot_panels.R` (the file
the rows themselves use), re-derives every row's GP set from the published
workbook, and diffs it against what `structure_plot_record.R` writes into
`output/structure_plot_record/` -- GP sets, each GP's maximum AUC in its
lineage, each row's palette (against the real `structure_plot_row_colors()`, not
a copy of its rule), and the clusters drawn and those omitted for being under
100 cells. Both inputs can be redirected (`--record-dir=`, `--table=`), which is
how the failing path is exercised. Its two other checks -- the assembled figure
on disk and the caption's per-row GP counts -- went with the figure on
2026-09-10.
...
**The per-lineage structure plots** were never published, so `verify_panels.sh`
never had a pair for them; `verify_structure_plot_gps.R` (above) stands in, and
still does now that only the record survives. While the figure existed, its
assembled PDF was pixel-compared against
`experiments/structure_plot_recolor/s5_per_lineage_glasbey.pdf`, the recolouring
trial the 2026-09-02 palette change adopted: **identical (RMSE 0)**. That trial
in turn rendered its control variant identical to the pre-change figure, so the
chain back to the exploratory `experiments/giant_structure_plot_by_lineage/`
rows was accounted for: cells, clusters, GP sets and row geometry never
changed, only colour.
...
- **Figure S4** (the published Figure 6b; renumbered repeatedly, S4 since 2026-09-10)
...
The single exception is **Figure 8/8B**: `Figure8b_plot.py` writes the published
PDF and `analysis/assets/Figure8/8B.png` from the same matplotlib figure in one
...
`render_site_assets.sh` therefore skips all of `Figure 8/`.

### Panels rasterized by `scattermore`

The ten protein-gate vs. GP-loading panels (**7e-7j** and **s6c-s6f**) draw a
...
`figs5_gating` in `FigureS5.R`) and the loops iterate over its *names*, so a GP
...
## Note on re-running FigureS5.R (and FigureS4.R)
...
non-write into a hard error. `FigureS4.R`, split out of it on 2026-07-30, writes
...
the current numbering is Extended Data Figure **6**, not our Figure S5
(CD69-associated GPs and gating, which *is* produced here by `FigureS5.R`).

Confirmed via PDF `/Creator` metadata during this refactor:

- Figure 7, panel A is a **Figma** schematic (not code-generated).
- Figure 7, panels C/D/F and **all 11** published Figure S7 panels are **Matplotlib**
...
two half-panels into a single PDF. It is built by `Figure8b.R` ->
`Figure8b_rematch.py` -> `Figure8b_plot.py`, documented on
